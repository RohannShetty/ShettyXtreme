    ...
    async def _on_bar(self, event: Event) -> None:
        ...
        if len(history) >= self.lookback + 1:
            ...
            if results:
                ...
                logger.debug(
                    "Breakout scan for %s: %d result(s)", bar.symbol, len(results)
                )
    ...
    def _scan_symbol(
        ...
        if resistance > 0:
            ...
            if breakout_pct >= self.threshold_pct and current_bar.close > resistance:
                confidence = self._compute_confidence(
                    breakout_pct, current_bar.volume, avg_volume, direction="bullish"
                )
                results.append({
                    "symbol": symbol,
                    "direction": "bullish",
        ...
        if support > 0:
            ...
            if breakdown_pct >= self.threshold_pct and current_bar.close < support:
                confidence = self._compute_confidence(
                    breakdown_pct, current_bar.volume, avg_volume, direction="bearish"
                )
                results.append({
                    "symbol": symbol,
                    "direction": "bearish",
