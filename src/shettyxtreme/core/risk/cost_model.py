...
Canonical home for CostBreakdown — imported by intelligence/ and integration/.
"""
...
def compute_cost(
    ...
    Args:
        ...
        lot_size: Contracts per lot.  When ``None``, falls back to
            ``quantity`` (i.e. assumes 1 lot).
    ...
    effective_lot = lot_size if lot_size is not None and lot_size > 0 else quantity
    num_lots = max(1, quantity // effective_lot) if effective_lot > 0 else 1
