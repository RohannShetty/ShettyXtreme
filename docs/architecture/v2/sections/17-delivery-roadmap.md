...
| 3 | Advanced intelligence | **3A DONE (2026-08-01); 3B DONE (2026-08-01)** | 3A: session-gated shadow graduation (≥20 sessions, direction-aware, spec semantics), calibration→sizing, correlation block caps + conviction D/P/G live, walkforward breakdowns, learning endpoints — 563 tests green. 3B: research workspace core — DeepSeek briefer harness (3 lenses), schema-validated briefs with reject-retry, human approve/reject + outcome stub, `/api/research/*` — 599 tests green. 3C (read-only data tools/MCP, critic pass, terminal panel): next |
