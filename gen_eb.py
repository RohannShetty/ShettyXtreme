...
async def test_subscribe_and_publish(event_bus):
    handler = AsyncMock()
    topic = event_bus.Topic.MARKET_DATA_TICK
...
async def test_unsubscribe(event_bus):
    handler = AsyncMock()
    topic = event_bus.Topic.SIGNAL_GENERATED
    event_bus.subscribe(topic, handler)
    event_bus.unsubscribe(topic, handler)

    event = event_bus.Event(topic=topic, data={"signal": "BUY"}, source="test")
    await event_bus.publish(event)
...
async def test_multiple_subscribers_same_topic(event_bus):
    handler1 = AsyncMock()
    handler2 = AsyncMock()
    topic = event_bus.Topic.ORDER_FILLED
    event_bus.subscribe(topic, handler1)
    event_bus.subscribe(topic, handler2)

    event = event_bus.Event(topic=topic, data={"order_id": "123"}, source="test")
    await event_bus.publish(event)
...
async def test_error_in_one_handler_does_not_block_others(event_bus):
    ...
    topic = event_bus.Topic.RISK_ALERT
    event_bus.subscribe(topic, failing_handler)
    event_bus.subscribe(topic, good_handler)

    event = event_bus.Event(topic=topic, data={"message": "risk"}, source="test")
    await event_bus.publish(event)
...
async def test_publish_nowait(event_bus):
    handler = AsyncMock()
    topic = event_bus.Topic.SYSTEM_STATUS
    event_bus.subscribe(topic, handler)

    event = event_bus.Event(topic=topic, data={"status": "ok"}, source="test")
...
async def test_subscriber_count(event_bus):
    ...
    event_bus.subscribe(event_bus.Topic.MARKET_DATA_TICK, handler1)
    assert event_bus.subscriber_count == 1

    event_bus.subscribe(event_bus.Topic.ORDER_PLACED, handler2)
...
async def test_different_topic_no_delivery(event_bus):
    handler = AsyncMock()
    event_bus.subscribe(event_bus.Topic.ORDER_PLACED, handler)

    event = event_bus.Event(
        topic=event_bus.Topic.MARKET_DATA_TICK, data={}, source="test",
    )
    await event_bus.publish(event)
...
os.makedirs("tests/core", exist_ok=True)
with open("tests/core/test_event_bus.py", "w") as f:
    f.write(content)
print("test_event_bus.py written")
